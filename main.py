...
class BrainDQNMain(object):
    ...
    def __init__(self, actions):
        ...
        self.Q_net = DeepNetWork().to(device)  # Move model to GPU
        self.Q_netT = DeepNetWork().to(device);  # Move model to GPU
        self.load()
    ...
    def train(self): # Step 1: obtain random minibatch from replay memory
        ...
        nextState_batch = torch.Tensor(nextState_batch).to(device)  # Move to GPU
        action_batch = np.array(action_batch)
        index = action_batch.argmax(axis=1)
        print("action " + str(index))
        index = np.reshape(index, [BATCH_SIZE, 1])
        action_batch_tensor = torch.LongTensor(index).to(device)  # Move to GPU
        ...
        state_batch_tensor = Variable(torch.Tensor(state_batch)).to(device)  # Move to GPU
        y_batch_tensor = Variable(torch.Tensor(y_batch)).to(device)  # Move to GPU
    ...
    def getAction(self):
        currentState = torch.Tensor([self.currentState]).to(device)  # Move to GPU
